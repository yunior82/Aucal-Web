...
$query = mysqli_query($conexion, 'SELECT COUNT(1) AS count FROM mv_articles WHERE status = 1 AND category_id = 2');
...
$query = mysqli_query($conexion, 'SELECT id, title, subtitle, meta_description, created_at, image_url, content, slug FROM mv_articles WHERE status = 1 AND category_id = 2 ORDER BY created_at DESC, id LIMIT ' . (($page - 1) * 6) . ', 6');
...
echo $renderer->render('blog-actualidad.twig', ['articles' => $articles, 'featured' => $featured, 'pages' => pagination($page, $lastPage, 5)]);
die();